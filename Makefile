...
#
# Layout B (dependent): own sources live in src/; sml-bigint is vendored under
...
SRCS       := $(wildcard $(BIGINTDIR)/*.sig $(BIGINTDIR)/*.sml src/* test/*.sml) $(TEST_MLB)
...
# its own. Load the vendored sml-bigint first, then the numbertheory sources,
# then the test driver.
poly test-poly:
	printf 'use "$(BIGINTDIR)/bigint.sig";\nuse "$(BIGINTDIR)/bigint.sml";\nuse "src/numbertheory.sig";\nuse "src/numbertheory.sml";\nuse "test/harness.sml";\nuse "test/test_numbertheory.sml";\nuse "test/entry.sml";\nuse "test/main.sml";\n' | $(POLY) -q --error-exit
